  ...
  return (
    <Box flex={1} backgroundColor="primary3">
      <Header
        ...
        renderRight={() => (
          <TouchableOpacity
            onPress={() => navigation.navigate('MenuSearch', {item: item})}>
            <Image source={Search} />
          </TouchableOpacity>
        )}
      />
      ...
      {search ? (
        ...
        <Box>
          <Box flexDirection="row" ml="l" alignItems="center">
            <Image source={Location} />
            <Text variant="support220regular" pl="l">
              {item.name || ''}
            </Text>
          </Box>
          <Text variant="support412regular" pt="l" paddingHorizontal="l">
            CATEGORIES
          </Text>
          <FlatList
            contentContainerStyle={styles.MenuCategoriesStyles}
            data={category}
